...
- **断点续传**：单文件传输中断后重新运行，自动从已完成的 1 MB 块边界继续，无需重传
- **1 MB AEAD 分块**：加密与 I/O 并行，兼顾吞吐量与大文件传输效率
...
1. 在两台机器上分别选择各自要参与双机同步的源目录。
2. 在任意一台机器上选择对端设备地址，点击“首次完全同步”完成初次对齐。
3. 当前版本的“自动同步”仅用于本机把源目录实时镜像到本地目标目录，不会把变更持续推送到另一台机器。
4. 同步日志会显示扫描、比对、传输、收尾阶段，以及“复用缓存/重算哈希”统计，便于判断本次同步主要耗时在哪一段。
